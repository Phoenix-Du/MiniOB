...
RC PhysicalPlanGenerator::create_plan(UpdateLogicalOperator &update_oper, unique_ptr<PhysicalOperator> &oper, Session* session)
{
  ...
  // 将UpdateLogicalOperator的字段和值转换为UpdatePhysicalOperator需要的格式
  std::vector<std::string> field_names;
  field_names.push_back(update_oper.attribute_name());

  std::vector<Value> values;
  for (int i = 0; i < update_oper.value_amount(); i++) {
    values.push_back(update_oper.values()[i]);
  }

  oper = unique_ptr<PhysicalOperator>(new UpdatePhysicalOperator(update_oper.table(), field_names, values));
